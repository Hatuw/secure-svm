...
template <class T> void AddScalar(Vec<T> &a, T b) {
  for (int i = 0; i < a.length(); i++) {
    a[i] += b;
  }
}

template <class T> void AddScalar(Mat<T> &a, T b) {
...
template <class T> static T Sum(Vec<T> &a) {
  T val;
  val = 0;
  for (int i = 0; i < a.length(); i++) {
    val += a[i];
...
template <class T> static void FilterMatRows(Mat<ZZ_p> &a, Vec<T> &filt) {
...
template <class T> static void FilterMatCols(Mat<ZZ_p> &a, Vec<T> &filt) {
...
template <class T1, class T2> static void FilterVec(Vec<T1> &a, Vec<T2> &filt) {
...
template <class T> static inline void Init(Vec<T> &a, int n) {
  a.SetLength(n);
  clear(a);
}

template <class T> static inline void Init(Mat<T> &a, int nrow, int ncol) {
  a.SetDims(nrow, ncol);
  clear(a);
}

template <class T> static inline void ReshapeMat(Mat<T> &b, T &a) {
  b.SetDims(1, 1);
  b[0][0] = a;
}
...
static inline vector<string> load_suffixes(string fname) {
  ...
  if (!ifs.is_open()) {
    cout << "Could not open suffix file: " << fname << endl;
