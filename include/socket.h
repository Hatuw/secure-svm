...
public:
  ...
  bool Bind(uint16_t nPort=0, string ip = "") {
  ...
  bool Connect(string ip, uint16_t port, int32_t lTOSMilisec = -1) {
    sockaddr_in sockAddr;
    memset(&sockAddr,0,sizeof(sockAddr));
    sockAddr.sin_family = AF_INET;
    sockAddr.sin_addr.s_addr = inet_addr(ip.c_str());
  ...
  int64_t SendSecure(unsigned char *pBuf, int64_t nLen, int nFlags = 0) {
    ...
    if (!Encrypt(pBuf, nLen)) {
      cerr << "CSocket:;SendSecure: encryption failed" << endl;
      return -1;
    } else {
  ...
  bool SetKey(string keyfile) {
    ifstream ifs(keyfile.c_str(), ios::binary);
    if (!ifs.is_open()) {
      cerr << "CSocket::SetKey: failed to open key file " << keyfile << endl;
      return false;
    }

    if (!ifs.read((char *)key, PRF_KEY_BYTES)) {
      cerr << "CSocket::SetKey: failed to read " << PRF_KEY_BYTES << 
              " bytes from " << keyfile << endl;
      return false;
    }
